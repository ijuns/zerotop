...
To select Claude, use `infra/kubernetes/overlays/anthropic` as the starting overlay, replace its model placeholder, and provide `codegate/production/model-gateway/anthropic:api-key` through the restricted SecretStore. It switches `MODEL_PROVIDER`, the ExternalSecret key, and the sole provider FQDN egress rule together:

```powershell
kubectl kustomize .\infra\kubernetes\overlays\anthropic
```
