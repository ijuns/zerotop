...
3. Create the namespaced `SecretStore` resources documented in `infra/server/SECRET-CONTRACTS.md`. Most checked-in `ExternalSecret` resources are provider-neutral; the model-gateway contract is deliberately Anthropic-specific and maps only its internal token plus `ANTHROPIC_API_KEY`. The private overlay supplies only the restricted store identities and may add the optional NVD API key. Shared peer tokens resolve from canonical remote paths so both consumers rotate together. Do not commit rendered Secret values or expose a store to learner namespaces.
...
Claude is the production base's only model provider. Replace the `ANTHROPIC_MODEL` placeholder with the approved Claude model and provide `codegate/production/model-gateway/anthropic:api-key` through the restricted `SecretStore`. The base keeps the API key out of manifests and limits model-gateway provider egress to `api.anthropic.com:443`.
