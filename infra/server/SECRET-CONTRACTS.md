...
The deployment bases expect the following Secrets. Checked-in provider-neutral `ExternalSecret` resources fix their target keys and remote path contracts; a private overlay supplies only provider-specific namespaced `SecretStore` objects and identities.
...
| `codegate-model-gateway-secrets` | `codegate-platform` | `MODEL_GATEWAY_INTERNAL_TOKEN` and exactly one selected upstream key: `OPENAI_API_KEY` in the base or `ANTHROPIC_API_KEY` in the Anthropic overlay |
...
Shared relationship tokens always resolve from `codegate/production/internal/<relationship>` property `token`; the same canonical remote value is projected only into the peer Secrets listed above. Service data credentials use `codegate/production/<service>/postgres`, `/redis`, or `/elasticsearch` as applicable. The AI↔gateway credential is `codegate/production/internal/model-gateway:token`, while the selected upstream API key is isolated at `codegate/production/model-gateway/openai:api-key` or `codegate/production/model-gateway/anthropic:api-key`. Provider overlays project only the selected key and allow only its exact API FQDN. The optional `NVD_API_KEY` is added by a private overlay only when provisioned. Desktop session signing uses `codegate/production/desktop/session-signing`, and OpenVPN encryption/PKI use `codegate/production/openvpn/encryption` and `codegate/production/openvpn/pki`.
