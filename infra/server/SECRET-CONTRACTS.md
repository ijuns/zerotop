...
| `codegate-model-gateway-secrets` | `codegate-platform` | `MODEL_GATEWAY_INTERNAL_TOKEN`, `OPENAI_API_KEY` |
...
Shared relationship tokens always resolve from `codegate/production/internal/<relationship>` property `token`; the same canonical remote value is projected only into the peer Secrets listed above. Service data credentials use `codegate/production/<service>/postgres`, `/redis`, or `/elasticsearch` as applicable. The AI↔gateway credential is `codegate/production/internal/model-gateway:token`, while the upstream API key is isolated at `codegate/production/model-gateway/openai:api-key`; the optional `NVD_API_KEY` is added by a private overlay only when provisioned. Desktop session signing uses `codegate/production/desktop/session-signing`, and OpenVPN encryption/PKI use `codegate/production/openvpn/encryption` and `codegate/production/openvpn/pki`.
